...
- educationId로 조회할 경우, 삭제되지 않은 스크립트만 조회되며 최신 버전이 반환됩니다.
...
6. **스크립트 조회**:
   - `GET /scripts/lookup`에서 educationId로 조회할 경우, 삭제되지 않은 스크립트만 조회되며 최신 버전이 반환됩니다.
   - 모든 조회 API는 삭제된 스크립트를 반환하지 않습니다.
