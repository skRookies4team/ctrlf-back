...
<<<<<<< HEAD
        ...
        for (int i = 0; i < departments.length; i++) {
            ...
            boolean exists = allEducations.stream()
                .anyMatch(e -> title.equals(e.getTitle()) && e.getDeletedAt() == null);
            if (exists) {
...
<<<<<<< HEAD
        ...
        // 2. 성희롱 예방 교육 (SEXUAL_HARASSMENT_PREVENTION) - edu_type: MANDATORY
        String sexualHarassmentTitle = "성희롱 예방 교육";
        List<Education> allEducations2 = educationRepository.findAll();
        boolean sexualHarassmentExists = allEducations2.stream()
            .anyMatch(e -> sexualHarassmentTitle.equals(e.getTitle()) && e.getDeletedAt() == null);
        if (!sexualHarassmentExists) {
            Education edu = new Education();
            edu.setTitle(sexualHarassmentTitle);
            edu.setCategory(EducationTopic.SEXUAL_HARASSMENT_PREVENTION);
            edu.setDescription("직장 내 성희롱 예방 및 대응 방법에 대한 법정 필수 교육입니다.");
            edu.setPassScore(80);
            edu.setPassRatio(90);
            edu.setRequire(Boolean.TRUE);
            edu.setEduType(EducationCategory.MANDATORY);
            edu.setVersion(1);
            edu.setStartAt(Instant.now().minusSeconds(86400 * 60));
            edu.setEndAt(Instant.now().plusSeconds(86400 * 120));
            edu.setDepartmentScope(new String[]{"전체 부서"});
            educationRepository.save(edu);
            log.info("Created education: {}", sexualHarassmentTitle);
        } else {
            log.debug("Education already exists, skipping: {}", sexualHarassmentTitle);
        }

        // 3. 개인정보 보호 교육 (PERSONAL_INFO_PROTECTION) - edu_type: MANDATORY
        String personalInfoTitle = "개인정보 보호 교육";
        List<Education> allEducations3 = educationRepository.findAll();
        boolean personalInfoExists = allEducations3.stream()
            .anyMatch(e -> personalInfoTitle.equals(e.getTitle()) && e.getDeletedAt() == null);
        if (!personalInfoExists) {
            Education edu = new Education();
            edu.setTitle(personalInfoTitle);
            edu.setCategory(EducationTopic.PERSONAL_INFO_PROTECTION);
            edu.setDescription("개인정보 보호법에 따른 개인정보 취급 및 보호에 관한 법정 필수 교육입니다.");
            edu.setPassScore(80);
            edu.setPassRatio(90);
            edu.setRequire(Boolean.TRUE);
            edu.setEduType(EducationCategory.MANDATORY);
            edu.setVersion(1);
            edu.setStartAt(Instant.now().minusSeconds(86400 * 90));
            edu.setEndAt(Instant.now().plusSeconds(86400 * 90));
            edu.setDepartmentScope(new String[]{"전체 부서"});
            educationRepository.save(edu);
            log.info("Created education: {}", personalInfoTitle);
        } else {
            log.debug("Education already exists, skipping: {}", personalInfoTitle);
        }

        // 4. 직장 내 괴롭힘 예방 교육 (WORKPLACE_BULLYING) - edu_type: MANDATORY
        String workplaceBullyingTitle = "직장 내 괴롭힘 예방 교육";
        List<Education> allEducations4 = educationRepository.findAll();
        boolean workplaceBullyingExists = allEducations4.stream()
            .anyMatch(e -> workplaceBullyingTitle.equals(e.getTitle()) && e.getDeletedAt() == null);
        if (!workplaceBullyingExists) {
            Education edu = new Education();
            edu.setTitle(workplaceBullyingTitle);
            edu.setCategory(EducationTopic.WORKPLACE_BULLYING);
            edu.setDescription("직장 내 괴롭힘 예방 및 대응 방법에 대한 법정 필수 교육입니다.");
            edu.setPassScore(80);
            edu.setPassRatio(90);
            edu.setRequire(Boolean.TRUE);
            edu.setEduType(EducationCategory.MANDATORY);
            edu.setVersion(1);
            edu.setStartAt(Instant.now().minusSeconds(86400 * 120));
            edu.setEndAt(Instant.now().plusSeconds(86400 * 60));
            edu.setDepartmentScope(new String[]{"전체 부서"});
            educationRepository.save(edu);
            log.info("Created education: {}", workplaceBullyingTitle);
        } else {
            log.debug("Education already exists, skipping: {}", workplaceBullyingTitle);
        }

        // 5. 장애인 인식 개선 교육 (DISABILITY_AWARENESS) - edu_type: MANDATORY
        String disabilityAwarenessTitle = "장애인 인식 개선 교육";
        List<Education> allEducations5 = educationRepository.findAll();
        boolean disabilityAwarenessExists = allEducations5.stream()
            .anyMatch(e -> disabilityAwarenessTitle.equals(e.getTitle()) && e.getDeletedAt() == null);
        if (!disabilityAwarenessExists) {
            Education edu = new Education();
            edu.setTitle(disabilityAwarenessTitle);
            edu.setCategory(EducationTopic.DISABILITY_AWARENESS);
            edu.setDescription("장애인에 대한 인식 개선 및 편견 해소를 위한 법정 필수 교육입니다.");
            edu.setPassScore(80);
            edu.setPassRatio(90);
            edu.setRequire(Boolean.TRUE);
            edu.setEduType(EducationCategory.MANDATORY);
            edu.setVersion(1);
            edu.setStartAt(Instant.now().minusSeconds(86400 * 15));
            edu.setEndAt(Instant.now().plusSeconds(86400 * 165));
            edu.setDepartmentScope(new String[]{"전체 부서"});
            educationRepository.save(edu);
            log.info("Created education: {}", disabilityAwarenessTitle);
        } else {
            log.debug("Education already exists, skipping: {}", disabilityAwarenessTitle);
        }
